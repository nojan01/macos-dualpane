      ...
      <div class="helpbar">
        {SHORTCUTS.map((s) => (
          <span class="help-item">
            <kbd>{s.key === "__F3__" ? t("hb.keys.f3") : s.key}</kbd>{" "}
            {t(s.descKey)}
          </span>
        ))}
      </div>
...
export function FnBar() {
  return (
    <div class="fnbar">
      ...
      <button
        class="fn-btn"
        title={t("fn.preview.title")}
        onClick={() => void doQuickLook()}
      >
        <b>F3</b> {t("fn.preview")}
      </button>
      <button
        class="fn-btn"
        title={t("fn.copy.title")}
        onClick={() => void startTransfer("copy")}
      >
        <b>F5</b> {t("fn.copy")}
      </button>
      <button
        class="fn-btn"
        title={t("fn.move.title")}
        onClick={() => void startTransfer("move")}
      >
        <b>F6</b> {t("fn.move")}
      </button>
      <button
        class="fn-btn"
        title={t("fn.newFolder.title")}
      ...
      <button
        class="fn-btn danger"
        title={t("fn.delete.title")}
        onClick={(ev) => void deleteSelected(ev.shiftKey)}
      >
        <b>F8</b> {t("fn.delete")}
      </button>
      <button
        class="fn-btn"
        title={t("fn.zip.title")}
      ...
      <button
        class="fn-btn"
        title={t("fn.refresh.title")}
        onClick={() => void forceRefreshPane(state.active)}
      >
        <b>⌘R</b> {t("fn.refresh")}
      </button>
      <button
        class="fn-btn"
        title={t("fn.terminal.title")}
        onClick={() => {
